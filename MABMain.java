...
/**
 * Implementation of an epsilon greedy multi-armed bandit.
 *
 * @author abhinav.sunderrajan
 */
public class MABMain {

  private static final double BANDIT_PROB[] = {
    0.10, 0.50, 0.60, 0.80, 0.10, 0.25, 0.60, 0.45, 0.75, 0.65
  };
  ...
  private class Agent {
    ...
    private int getMaxQValAction() {
      ...
      for (int index = 0; index < Qval.length; index++) {
        if (Qval[index] == max) indices.add(index);
      }
      ...
      for (int action : action_history) {
        ...
        if (episode % 1000 == 0) {
          for (int bandit = 0; bandit < action_counts.length; bandit++)
            System.out.println(
                episode + "\t" + (bandit + 1) + "\t" + action_counts[bandit] / episode);
        }
      }
    }
  }
