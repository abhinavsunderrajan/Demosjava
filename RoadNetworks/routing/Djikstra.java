...
public class Djikstra extends RoutingAlgoAbstract {
	...
	public static void main(String args[]) throws Exception {
		...
		String mapVersion = "v1.1.41";
		String DIR_ROAD_FILES = "/Users/abhinav.sunderrajan/Desktop/road-speed-profiling/abhinav/road_network_files/";

		OSMRoadNetworkModel roadNetworkModel = new OSMRoadNetworkModel(
				DIR_ROAD_FILES + mapVersion + "/normalized/" + "roadsKUL" + mapVersion + ".txt",
				DIR_ROAD_FILES + mapVersion + "/normalized/" + "nodesKUL" + mapVersion + ".txt", 1, 6, false, true);

		Djikstra rd = new Djikstra(roadNetworkModel);
		long t1 = System.currentTimeMillis();

		// RoadNode begin =
		// roadNetworkModel.getAllRoadsMap().get(54139766l).getBeginNode();
		// RoadNode end =
		// roadNetworkModel.getAllRoadsMap().get(608624192l).getEndNode();
		RoadNode begin = roadNetworkModel.getAllNodes().get(665897498l);
		RoadNode end = roadNetworkModel.getAllNodes().get(4725377795l);

		rd.djikstra(begin, end);
		System.out.println("Execution time: " + (System.currentTimeMillis() - t1) + " ms visited node size: "
				+ rd.visited.size() + " " + rd.getRoute());
	 ...
	 * @param road
	 * @param roadNode
	 * @return the array with first element is the distance to the begin node
	 *         and the other is the distance to the end node.
	 */
